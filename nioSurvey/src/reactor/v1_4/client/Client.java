...
//		socketChannel.socket().setReceiveBufferSize(1024);
		socketChannel.connect(new InetSocketAddress("192.168.249.5", 18080));
...
//		log.info("start down");
		...
		while (true) {
			...
			average_time += tem;
			succ_num++;
			/*log.info*/System.out.println(Thread.currentThread().getName()+" wasteTime "+tem+" ,fail "+fail_num+", average_time "+average_time/succ_num);
	}
